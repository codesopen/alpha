    ...
    if cd $dir_log/$task_name &>/dev/null && [[ $(ls) ]]; then
        ...
        if [[ ${#code[*]} -gt 0 ]]; then
            for ((m = 0; m < ${#pt_pin[*]}; m++)); do
                ...
                echo "输入到code日志:$config_name_my$j='$tmp_my_code'"
                echo -e "$config_name_my$j='$tmp_my_code'\n" >> $latest_log_path
            done
        else
            echo "## 从日志中未找到任何互助码"
        fi
            ...
            4) ## 本套脚本内指定前 N 个账号优先助力，N 个以后账号间随机助力(随机部分账号顺序固定)。
                ...
                for ((m = 0; m < ${#pt_pin[*]}; m++)); do
                    ...
                    for ((n = 0; n < $user_sum; n++)); do
                        ...
                        k=$((n + 1))
                        if [[ $k -le $front_num ]]; then
                        	    if [[ "$tmp_for_other" = "" ]]; then
                        	    		tmp_for_other="\${$config_name_my$k}"
                        	    else
                        	    		tmp_for_other="$tmp_for_other@\${$config_name_my$k}"
                        	    fi
                        fi
                    ...
                    echo "输入到code日志:$config_name_for_other$j='$tmp_for_other$tmp_ramdom_for_other'"
                    echo -e "$config_name_for_other$j='$tmp_for_other$tmp_ramdom_for_other'\n" >> $latest_log_path
                done
                ;;
...
help_codes(){
    ...
    if [ ! -f $ShareCode_log ] || [ -z "$(cat $ShareCode_log | grep "^$config_name_my\d")" ]; then
         echo -e "\n## $chinese_name\n## 互助码格式：ForOther${config_name_my}*=\${${config_name_my}1}@\${${config_name_my}2}\n${config_name_my}1=''\n" >> $ShareCode_log
    fi
    echo -e "\n#【`date +%X`】 正在更新 $chinese_name 的互助码..."
    for ((i=1; i<=200; i++)); do
        local new_code="$(cat $latest_log_path | grep "^$config_name_my$i=.\+'$" | head -1 | sed "s/\S\+'\([^']*\)'$/\1/")"
